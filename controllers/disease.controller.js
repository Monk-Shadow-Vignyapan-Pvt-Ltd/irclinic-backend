...
export const addDisease = async (req, res) => {
    try {
        const { diseaseName, diseaseDescription,parentID, rank, imageBase64, userId, diseaseURL,
        ...
        res.status(500).json({ message: 'Failed to upload disease', success: false });
    }
};
...
export const getDiseases = async (req, res) => {
    try {
        ...
        if (!diseases) return res.status(404).json({ message: "diseases not found", success: false });
...
export const getDiseaseName = async (req, res) => {
    try {
        ...
        const diseases = await Disease.find({ parentID: "" }).select("_id diseaseName diseaseURL");
...
export const getDiseaseById = async (req, res) => {
    try {
        ...
        if (!disease) return res.status(404).json({ message: "disease not found!", success: false });
...
export const updateDisease = async (req, res) => {
    try {
        const { id } = req.params;
        const { diseaseName, imageBase64, rank, diseaseDescription, parentID, userId, diseaseURL,
            seoTitle, seoDescription, } = req.body;
        ...
        if (existingDisease.diseaseURL && existingDisease.diseaseURL !== diseaseURL && !oldUrls.includes(existingDisease.diseaseURL)) {
        ...
        const disease = await Disease.findByIdAndUpdate(id, updatedData, { new: true, runValidators: true });
        if (!disease) return res.status(404).json({ message: "Disease not found!", success: false });
        return res.status(200).json({ disease, success: true });
    } catch (error) {
        console.log(error);
...
export const deleteDisease = async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ message: 'Failed to delete disease', success: false });
    }
};
